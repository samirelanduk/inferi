...
class Data:
    ...
    def add(self, value):
        """Adds a value to the Data - specifically a y value. The x value in the
        pair will be generated by incrementing the highest integer x value
        currently in the Data by 1 and using that.

        If you know the x value you want to use you are probably better using
        ``data[x] = y`` syntax.

        :param value: The value to add."""

        x = sorted(filter(lambda k: isinstance(k, int), self.xvalues()))
        self._values.append([x[-1] + 1, value])
